...
example {m n : ℕ} (coprime_mn : m.Coprime n) : m ^ 2 ≠ 2 * n ^ 2 := by
  ...
  have : 2 ∣ m.gcd n := by
    ...
    · assumption
  have : 2 ∣ 1 := by
    have : m.gcd n = 1 := by exact coprime_mn
    rw [← this]
    assumption
  norm_num at this
...
example {m n p : ℕ} (nnz : n ≠ 0) (prime_p : p.Prime) : m ^ 2 ≠ p * n ^ 2 := by
  ...
  have eq2 : (p * n ^ 2).factorization p = 2 * n.factorization p + 1 := by
    have : p.factorization p = 1 := by
      exact prime_p.factorization'
    rw [← this, add_comm]
    have : (n ^ 2).factorization p = 2 * n.factorization p := by
      exact factorization_pow' n 2 p
    rw [← this]
    have pne0 : p ≠ 0 := by
      have := Nat.Prime.two_le prime_p
      linarith
    exact factorization_mul' pne0 nsqr_nez p
...
example {m n k r : ℕ} (nnz : n ≠ 0) (pow_eq : m ^ k = r * n ^ k) {p : ℕ} :
  ...
  have eq2 : ((r + 1) * n ^ k).factorization p =
    ...
    have : (n ^ k).factorization p = k * n.factorization p := by
      exact factorization_pow' n k p
    rw [← this]
    have rne0 : r + 1 ≠ 0 := by
      linarith
    have nsqr_nez : n ^ k ≠ 0 := by
      simp
      intro neq
      contradiction
    rw [add_comm ((n ^ k).factorization p)]
    exact factorization_mul' rne0 nsqr_nez p
