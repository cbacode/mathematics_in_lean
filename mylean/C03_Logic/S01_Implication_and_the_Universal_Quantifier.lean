...
example (of : FnOdd f) (og : FnOdd g) : FnEven fun x ↦ f x * g x := by
  intro x
  calc
    (fun x ↦ f x * g x) x = f x * g x := by dsimp
    _ = (-f (-x)) * (-g (-x)) := by rw [of, og]
    _ = f (-x) * g (-x) := by exact neg_mul_neg (f (-x)) (g (-x))
...
example (ef : FnEven f) (og : FnOdd g) : FnEven fun x ↦ f (g x) := by
  ...
  dsimp
  calc
    f (g x) = f (-g x) := by apply ef
    _ = f (g (-x)) := by rw[og, neg_neg]
...
-- solution
theorem Subset.trans_ans : r ⊆ s → s ⊆ t → r ⊆ t :=
  fun rsubs ssubt x xr ↦ ssubt (rsubs xr)

theorem Subset.trans2 : r ⊆ s → s ⊆ t → r ⊆ t := by
  intro rs st x xr
  apply st
  apply rs
  exact xr
