...
lemma dia_inv [Group₁ G] (a : G) : a ⋄ a⁻¹ = 𝟙 := by
  have : a⁻¹ ⋄ a = 𝟙 := (inv_dia a)
  have := inv_eq_of_dia this
  nth_rw 1 [← this]
  exact inv_dia a⁻¹
...
lemma Group₃.mul_inv {G : Type} [Group₃ G] {a : G} : a * a⁻¹ = 1 := by
  have : a⁻¹ * a = 1 := (inv_mul a)
  have := inv_eq_of_mul this
  nth_rw 1 [← this]
  exact inv_mul a⁻¹
...
lemma mul_right_cancel₃ {G : Type} [Group₃ G] {a b c : G} (h : b*a = c*a) : b = c := by
  rw [← mul_one b, ← Group₃.mul_inv, ← mul_assoc₃, h, mul_assoc₃, Group₃.mul_inv, mul_one]
