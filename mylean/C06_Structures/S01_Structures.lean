...
def weightedAverage (n : ℕ) (a b : StandardSimplex n) (lambda : Real) (lambda_nonneg : 0 ≤ lambda) (lambda_le : lambda ≤ 1): StandardSimplex n
    where
  V i := lambda * a.V i + (1 - lambda) * b.V i
  NonNeg := by
    intro i
    simp
    have one_nonneg : 0 ≤ 1 - lambda := by linarith
    apply add_nonneg
    · apply mul_nonneg lambda_nonneg (a.NonNeg i)
    · apply mul_nonneg one_nonneg (b.NonNeg i)
  sum_eq_one := by
    simp [div_eq_mul_inv, ← Finset.mul_sum, Finset.sum_add_distrib,
      a.sum_eq_one, b.sum_eq_one]
